...
class AES_SYSTEM():
    ...
    def Encrypt(self):
        for self.address in UIWindow.files_list:
            ...
            if UIWindow.enc_key_input.text():  # KEY
                if UIWindow.enc_key_label.text() == 'Key (B):':  # Bytes format key
                    ...
                    if len(UIWindow.enc_key_input.text()) <= 31:
                        self.key = pad(UIWindow.enc_key_input.text().encode(),
                                       32)  # pad key in total of 32 bytes (256bit)
            ...
            UIWindow.enc_files_counter_progressBar.setFormat(
                f'{UIWindow.files_counter}/{UIWindow.enc_files_counter_progressBar.maximum()}')  # files counter bar 0/$num_of_files
            ...
            UIWindow.enc_files_counter_progressBar.setFormat(
                f'{UIWindow.files_counter}/{UIWindow.enc_files_counter_progressBar.maximum()}')
    ...
    def Decrypt(self):
        for self.address in UIWindow.files_list:
            ...
            ## KEY & NONCE inputs
            if not (
                    UIWindow.dec_key_input.text() and UIWindow.dec_nonce_input.text()):  # If key/nonce have not been filled, then stop
            ...
            UIWindow.dec_files_counter_progressBar.setFormat(
                f'{UIWindow.files_counter}/{UIWindow.dec_files_counter_progressBar.maximum()}')  # files counter bar 0/$num_of_files
            ...
            UIWindow.dec_files_counter_progressBar.setFormat(
                f'{UIWindow.files_counter}/{UIWindow.dec_files_counter_progressBar.maximum()}')
        ...
        if self.DBFileChecker():
            with open(UIWindow.DATABASE_FILE, 'r+') as DB_file:
                try:
                    ...
                    data[self.filehash] = [self.key.hex(), self.nonce.hex(), self.tag.hex(),
                                           UIWindow.Enc_system_label.text(), self.address]
                    DB_file.seek(0)
            ...
            if len(address_list) == 1:  # If only one file has been selected
                ...
                if self.filesize < 64:  # If file is >= 64GB, you should split file to smaller parts cause of encryption security reasons
                    ...
                    if self.address.endswith(
                            UIWindow.FILE_EXT):  # If file is already encrypted, check if it's key/nonce exist in DB
                        self.filehash = self.sha256Hash(self.address)
                        self.KeyNonceSearcher()
                ...
                if all([os.path.getsize(x) / pow(1024, 3) < 64 for x in
                        self.addresses]):  # Check if all files are <64GB
...
class RamToUse(QWidget):
    ...
    def initUI(self):  # https://zetcode.com/pyqt/qslider/
        ...
        self.slide_bar.setRange(1, (
                    self.free_mb - self.free_mb // 10))  # Let free at least 10% of the free RAM MB Minimum 65MB of RAM Need
...
class UI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(
            f'crypt V{str(EX_Version)}')  # Window Title and Version Number It Will Provide By master Dev.
        self.setWindowIcon(QIcon('images/Small_Logo.png'))  # Window icon

        ### Default variables ###
        self.DATABASE_FILE = None  # Default key/nonce database
        self.FILE_EXT = '.encex'
        self.FEEDBACKURL = 'https://forms.gle/heP2uenQrkV11fvf9'  # Google From will collect Feedback from Customer.
        ...
        self.option_Blocks_Size.triggered.connect(RamToUse.RamToUseOpenWindow)  # Usable RAM
    ...
    def LabelSwitcher(self, label_name):
        if label_name == 'enc_key_label':
            if self.enc_key_label.text() == 'Key (B):':
                ...
                self.enc_key_input.setPlaceholderText('Type your key in Bytes format (example: Ajay123)')
            ...
            if self.dec_key_label.text() == 'Key (B):':
                ...
                self.dec_key_input.setPlaceholderText('Type your key in Bytes format (example: Ajay123)')
    ...
    def SetDefaultOptions(self):
        ...
        json.decoder.JSONDecodeError, ValueError, FileNotFoundError):  # If Options.txt has a problem, then remake it
        ...
        self.option_Check_for_dublicate_key_nonce_in_DB.setChecked(
            options['option_Check_for_dublicate_key_nonce_in_DB'])
        self.option_not_decrypted_verified_keep_original_file.setChecked(
            options['option_not_decrypted_verified_keep_original_file'])
